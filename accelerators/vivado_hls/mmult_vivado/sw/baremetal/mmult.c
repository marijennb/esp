...
int main(int argc, char *argv[])
{
    ...
    if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
        ...
        in_words_adj  = round_up(2 * N * N, DMA_WORD_PER_BEAT(sizeof(token_t)));
        out_words_adj = round_up(N, DMA_WORD_PER_BEAT(sizeof(token_t)));
    }
    ...
    ndev = probe(&espdevs, VENDOR_SLD, SLD_MMULT, DEV_NAME);
    if (ndev == 0) {
        printf("mmult not found\n");
        return 0;
    }
    ...
    for (n = 0; n < ndev; n++) {
        ...
        for (coherence = ACC_COH_NONE; coherence <= ACC_COH_RECALL; coherence++) {
            ...
            /* <<--regs-config-->> */
			iowrite32(dev, MMULT_N_REG, N);
			iowrite32(dev, MMULT_SHIFT_REG, SHIFT);
